...
import DashboardLayout from "../components/DashboardLayout"; // ✅ unified layout
  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        ...
        <StatCard
          title="Available"
          value={all.filter((p) => p.isAvailable).length}
        />
        <StatCard
          title="Low Stock"
          value={all.filter((p) => p.stock <= 5).length}
        />
        <StatCard
          title="Signature Today"
          value={all.filter((p) => p.isSignatureToday).length}
        />
      </div>
              ...
              items.map((p) => (
                <tr key={p._id} className="border-t">
                  ...
                  <td className="px-4 py-3 flex gap-2">
                    <Button tone="ghost" onClick={() => openEdit(p)}>
                      Edit
                    </Button>
                    <Button tone="danger" onClick={() => remove(p.slug)}>
                      Delete
                    </Button>
                  </td>
                </tr>
