// pages/admin/products.tsx (or src/pages/admin/products.tsx)
import { useEffect, useMemo, useState } from "react";
import useSWR from "swr";
import { useSession, signIn, signOut } from "next-auth/react";

// --- fetcher (array response) ---
const fetcher = async (url: string) => {
  const r = await fetch(url, { credentials: "same-origin" });
  if (!r.ok) throw new Error(await r.text());
  return r.json(); // <-- plain array from your API
};
...
function TextArea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      className={`w-full rounded-2xl border border-neutral-300 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-black/10 ${props.className || ""}`}
    />
  );
}
...
export default function AdminProductsPage() {
  ...
  // client-side pagination (since API returns array)
  const [page, setPage] = useState(1);
  const limit = 10;

  // modal form state aligned with your Zod schema
  ...
  // build query for your API (no page param)
  const listUrl = useMemo(() => {
    const params = new URLSearchParams();
    if (q) params.set("q", q);
    if (category) params.set("category", category);
    if (signatureToday) params.set("signatureToday", "1");
    params.set("limit", "200"); // fetch more and paginate client-side
  ...
  const toArrayByLine = (s: string) => s.split("\n").map((x) => x.trim()).filter(Boolean);
  const toArrayByComma = (s: string) => s.split(",").map((x) => x.trim()).filter(Boolean);
  ...
  async function save() {
    ...
    // IMPORTANT: use slug when editing since API route is [slug].ts
    ...
    <div className="min-h-screen bg-neutral-50">
      ...
      <header className="sticky top-0 z-40 border-b border-neutral-200 bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <h1 className="text-lg font-semibold">Admin • Product Management</h1>
          <div className="flex items-center gap-2">
            <PrimaryButton onClick={openCreate} disabled={busy}>+ New Product</PrimaryButton>
            <button
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-4 grid gap-3 sm:grid-cols-5">
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              ...
              onChange={(e) => { setSignatureToday(e.target.checked); setPage(1); }}
            />
            Signature today only
          </label>
        ...
        <div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
            ...
            <table className="w-full border-collapse">
              <thead className="bg-neutral-50 text-left text-sm">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium text-neutral-600">Pricing</th>
                  <th className="px-4 py-3 font-medium text-neutral-600">Stock</th>
                  <th className="px-4 py-3 font-medium text-neutral-600">Flags</th>
                  <th className="px-4 py-3 font-medium text-neutral-600">Actions</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  <tr><td colSpan={5} className="p-8 text-center text-sm text-neutral-500">Loading…</td></tr>
                ) : error ? (
                  <tr><td colSpan={5} className="p-8 text-center text-sm text-rose-600">{(error as any).message || "Error loading products"}</td></tr>
                ) : items.length === 0 ? (
                  <tr><td colSpan={5} className="p-8 text-center text-sm text-neutral-500">No products found</td></tr>
                ) : (
                  items.map((p) => (
                    <tr key={p._id} className="border-t border-neutral-100 align-top">
                      <td className="px-4 py-4">
                        <div className="flex gap-3">
                          <div className="grid grid-cols-3 gap-1">
                            {(p.images || []).slice(0, 3).map((src, i) => (
                              // eslint-disable-next-line @next/next/no-img-element
                              <img key={i} src={src} alt={p.name} className="h-14 w-14 rounded-lg border object-cover" />
                            ))}
                          </div>
                          <div>
                      ...
                      <td className="px-4 py-4">
                        <div className="text-sm">
                          {p.promotion ? (
                            <div className="text-neutral-500 line-through">Rs. {(p.price + p.promotion).toFixed(2)}</div>
                          ) : null}
                          <div className="font-medium">Rs. {Number(p.price).toFixed(2)}</div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex flex-wrap gap-2">
                          ...
                          {p.isSignatureToday ? <Badge tone="success">signature</Badge> : null}
                          {p.spicyLevel > 0 ? <Badge>🌶 x{p.spicyLevel}</Badge> : null}
                        </div>
                      </td>

                      <td className="px-4 py-4">
                        <div className="flex flex-wrap gap-2">
                          <IconButton onClick={() => openEdit(p)} disabled={busy}>Edit</IconButton>
                          <IconButton onClick={() => toggleAvailable(p)} disabled={busy}>
          ...
          <div className="grid gap-3 p-3 md:hidden">
            {isLoading ? (
              ...
              <div className="p-6 text-center text-sm text-rose-600">{(error as any).message || "Error loading products"}</div>
            ) : items.length === 0 ? (
              <div className="p-6 text-center text-sm text-neutral-500">No products found</div>
            ) : (
              items.map((p) => (
                <div key={p._id} className="rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
                  <div className="font-medium">{p.name}</div>
                  <div className="text-xs text-neutral-500">/{p.slug}</div>
                  <div className="mt-2 text-sm">
                    {p.promotion ? (
                      <span className="mr-2 text-neutral-500 line-through">Rs. {(p.price + p.promotion).toFixed(2)}</span>
                    ) : null}
                    <span className="font-medium">Rs. {Number(p.price).toFixed(2)}</span>
                  </div>
        ...
        <div className="mt-4 flex items-center justify-between">
          <p className="text-sm text-neutral-500">
            ...
            <span className="font-medium text-neutral-800">{pages}</span> • {total} items
          </p>
      ...
      <Modal
        ...
        footer={
          <>
            <IconButton onClick={() => { setOpen(false); setEditing(null); }} disabled={busy}>Cancel</IconButton>
            <PrimaryButton onClick={save} disabled={busy}>{editing ? "Save Changes" : "Create"}</PrimaryButton>
          </>
        }
      >
        <div className="grid gap-3 sm:grid-cols-2">
          <TextInput placeholder="Name" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} />
          <TextInput
            placeholder="Slug (unique)"
            value={f.slug}
            onChange={(e) => setF({ ...f, slug: e.target.value })}
            disabled={!!editing} // lock slug while editing to match route param
          />
          <TextInput placeholder="Category" value={f.category} onChange={(e) => setF({ ...f, category: e.target.value })} />
          <TextInput placeholder="Price" type="number" value={f.price} onChange={(e) => setF({ ...f, price: e.target.value })} />
          <TextInput placeholder="Promotion (discount amount)" type="number" value={f.promotion} onChange={(e) => setF({ ...f, promotion: e.target.value })} />
          <TextInput placeholder="Stock" type="number" value={f.stock} onChange={(e) => setF({ ...f, stock: e.target.value })} />
          <TextInput placeholder="Spicy level (0-3)" type="number" value={f.spicyLevel} onChange={(e) => setF({ ...f, spicyLevel: e.target.value })} />
          ...
          <label className="flex items-center gap-2 text-sm sm:col-span-2">
            ...
            />
            Mark as signature dish today (max 4 enforced by server)
          </label>
        </div>
      </Modal>
    </div>
