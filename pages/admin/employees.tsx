...
type Payroll = { _id: string; employeeId: string; type: PayrollType; amount: number; date: string; note?: string; };
...
const today = () => { const d = new Date(); const mm = String(d.getMonth()+1).padStart(2,"0"); const dd = String(d.getDate()).padStart(2,"0"); return `${d.getFullYear()}-${mm}-${dd}`; };
const shift = (days:number) => { const d = new Date(); d.setDate(d.getDate()+days); const mm = String(d.getMonth()+1).padStart(2,"0"); const dd = String(d.getDate()).padStart(2,"0"); return `${d.getFullYear()}-${mm}-${dd}`; };

export default function EmployeeAdminPage() {
  const { data: employees, mutate: mutateEmp } = useSWR<Employee[]>("/api/employees", fetcher);
  ...
  async function seed() {
    ...
    const r = await fetch("/api/employees/seed", { method: "POST" });
    if (!r.ok) return alert(await r.text());
    mutateEmp(); mutatePayroll();
    alert("Seeded!");
  }
  ...
  async function addEmployee(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!body.name || !body.role || isNaN(body.baseSalary)) return alert("Fill all fields");
  ...
  async function recordPayroll(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!body.employeeId || !body.type || !body.amount || !body.date) return alert("Fill all fields");
    ...
    <div className="mx-auto max-w-7xl p-4 sm:p-8">
      <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-600">Manage employees and record payroll transactions.</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={seed} className={cls.btnGhost}>Seed Dummy DB</button>
        </div>
      </header>
      ...
      <section className="mt-6">
        <div className="mb-3 text-lg font-semibold">Payroll Summary (selected range)</div>
      ...
      <section className="mt-6">
        ...
        <form onSubmit={addEmployee} className={cls.card + " grid gap-2 sm:grid-cols-5"}>
          <input name="name" placeholder="Full name" className={cls.input} />
          <input name="role" placeholder="Role (e.g., Chef)" className={cls.input} />
          <input name="baseSalary" placeholder="Base Salary (LKR)" inputMode="numeric" className={cls.input} />
        ...
        <div className={cls.card}>
          <table className="w-full text-sm">
            <thead><tr className="text-left text-neutral-500">
              <th className="py-2">Name</th><th>Role</th><th>Base Salary</th><th></th></tr></thead>
            <tbody>
              {(employees||[]).map(e=>(
                <tr key={e._id} className="border-t">
                  ...
                  <td className="text-right">
                    <button onClick={()=>deleteEmployee(e._id)} className={cls.btnGhost}>Delete</button>
                  </td>
                </tr>
              ))}
              {(employees||[]).length===0 && <tr><td colSpan={4} className="py-2 text-neutral-500">No employees yet</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
      ...
      {/* Record payroll */}
      <section className="mt-6">
        <div className="mb-3 text-lg font-semibold">Record Payroll Transaction</div>
        <form onSubmit={recordPayroll} className={cls.card + " grid gap-2 sm:grid-cols-6"}>
          ...
          <input name="amount" placeholder="Amount (LKR)" inputMode="numeric" className={cls.input} />
          <input name="note" placeholder="Note (optional)" className={cls.input + " sm:col-span-2"} />
      ...
      {/* History */}
      <section className="mt-6">
        <div className="mb-3 text-lg font-semibold">Payroll History (range)</div>
        <div className={cls.card}>
          <table className="w-full text-sm">
            <thead><tr className="text-left text-neutral-500">
              <th className="py-2">Date</th><th>Employee</th><th>Type</th><th>Amount</th><th>Note</th></tr></thead>
            <tbody>
              {(payroll||[]).map(p=>{
                ...
                return (
                  <tr key={p._id} className="border-t">
                    ...
                    <td>{p.type}</td>
                    <td className={p.type==="deduction" ? "text-green-700" : "text-red-700"}>{sign}{fmt.format(p.amount)}</td>
                    <td>{p.note || "-"}</td>
                  </tr>
                );
              })}
              {(payroll||[]).length===0 && <tr><td className="py-2 text-neutral-500" colSpan={5}>No transactions in range</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
